...
function init_team_user {
  if [ -z ${TEAM_USER+x} ] || [ -z "$TEAM_USER" ]; then
    TEAM_USER="spark-dt";
  fi
  usermod -l $TEAM_USER spark
  # useradd --system --uid=285 --gid=spark $TEAM_USER
}
...
case "$1" in
  driver)
    ...
    exec $(switch_process_user) /usr/bin/tini -s -- "${CMD[@]}"
    ;;
  executor)
    shift 1
    CMD=(
